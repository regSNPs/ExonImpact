...
plotcoilasa<-function(){
  ...
  all_data_filter_target_region<-subset(all_data,asa_1>=24&asa_1<=46&ss_8>=0.72&ss_8<=0.9&disorder_12>0.5&label=="HGMD");
  #annotate("rect", xmin = 0.72, xmax = 0.9, ymin = 24, ymax = 46)
  hist(all_data_filter_target_region[,"disorder_12"]);
  write.table(all_data_filter_target_region,file="result/interest_region.tsv",sep="\t",row.names = FALSE,quote=FALSE,col.names = TRUE);
