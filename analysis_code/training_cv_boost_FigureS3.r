...
##for only frameshift indels
#neutral<-read_csv("data/1000_genome_indel/frameshift_indel/1000_genome_transcript_exon_id_frameshift_features.csv"
                  #,col_names=TRUE);
#neutral<-neutral[,colnames(hgmd)]
...
##only keep the exon which region length is multiple of 3. 
...
p<-ggplot(roc_data_select,aes(x=fpr,ymin=0,ymax=tpr))+geom_ribbon(fill="gray66")+#geom_area(fill="green")+
  ...
  ggtitle("Supplementary Figure 3 100 times boostrap and 10 cross-validation")+
  facet_wrap(~method_name)+  theme(text=element_text(size=7));

#output
pdf("result/Supplementary Figure 3 (cv&bootstrap).pdf",width=8,height=5);
print(p);
dev.off();

setEPS();
postscript("result/eps/Supplementary Figure 3 (cv&bootstrap).eps", width=8,height=5 );
print(p);
dev.off();
