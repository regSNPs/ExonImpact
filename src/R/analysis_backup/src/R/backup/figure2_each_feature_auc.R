...
plotFigure2<-function(){
  ...
  textplot2("Each feature's distribution and KS-test statistic",2); 
  popViewport();
  ...
  for(i in 2:ncol(goodFeatures)){
    ...
    par(new=TRUE, fig=gridFIG(),mar=c(2,2,4,1),cex=1);
...
pdf("result/Figure-2 (evaluate each feature).pdf",width=17,height=9);
plotFigure2();
dev.off();


setEPS();
postscript("result/eps/Figure-2 (evaluate each feature).eps",width=17,height=9,fonts=c("mono") );
plotFigure2();
dev.off();
