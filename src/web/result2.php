				...
				onchange="checkBox(this)">protein <input type="checkbox"
					...
					onchange="checkBox(this)">transcripts <br />go to a
						...
						id="coordiantePosition" value=""> <input type="button"
							name="go" id="gotoposition" value="Go"> <a
								href='userXML/test_1chrY-26951604-26951655_NM_020420.xml'>Download
									the XML datafile.</a> <a href=></a> <br> <br>
		</div>
		<p></p>
...
var selected = [];
    ...
    //$('#mapping').DataTable( {
    //    //"ajax": '/ExonImpact/'+query_file_name+'/'+query_file_name+'.mapping.json.tsv',
   //     "ajax": 'usr_input/'+query_file_name+'.predict.json.tsv',
    ...
    $('#example tbody').on('click', 'tr', function () {
		...
		$.ajax({ url: 'gen_xml.php',
			...
			success: function(output) {
				alert(output);
				browser_init("/ExonImpact2/usr_xml/"+output);

				},
				async:true
		});
		//browser_init("/usr_xml/"+file_name);
